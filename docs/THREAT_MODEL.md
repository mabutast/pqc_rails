...
| 米大統領令14412「量子レジリエンス・クロック」 | 2026年10月22日 | 各省庁は移行計画をOMB通達M-26-15に基づき提出(30/90/180日の節目期限を経て具体化) |
| G7サイバーセキュリティ作業部会 + CISA | — | 共同声明「Preparing for the Post-Quantum Era: A Call to Action」(2026年9月)で暗号資産インベントリの重要性を強調 |

参考: 国家量子戦略(改定版)は2026年12月中旬に公表予定です。米国内では大統領令14412に基づき、連邦ID基盤(FICAM・物理アクセス制御)や金融セクター向け(米財務省「Quantum-Readiness Task Force」)など、個別領域への展開が2026年8月以降相次いでいます。2026年9月のDigiCert「World Quantum Readiness Day」では、各国の移行規制の期限が「2030年」に収斂する一方、対象範囲(どのデータ・システムが規制対象か)は国・地域ごとに分裂しているとの指摘があり、pqc_railsが将来複数アルゴリズム(HQC等)へ切替可能な設計を維持しておく価値を裏付けています。

### CRQC実現時期の観測材料

「量子コンピュータがいつ暗号解読に使えるようになるか」について、決定的な予測は存在しません。以下は実現時期の前後を示唆する動き(ハードウェアの進展、業界の資金調達、基礎研究)の一部です。個別の項目には確認できた日付を付記しています。
...
- **256ビット楕円曲線離散対数問題(ECDLP-256)の解読に必要な論理量子ビット数の見積もりが、2026年に入り継続的に下方修正されています**。ECDLP-256は`HybridKem`が古典側で使うX25519の安全性根拠そのものです。
  ...
  - 2026年9月、IonQがtrapped-ion量子コンピュータによるECDLP-256(secp256k1)の完全コンパイル済み資源見積りを公表しました。物理量子ビット19,397個・論理量子ビット1,457個で26日程度での解読を試算するものです(arXiv投稿、2026年9月時点で未査読)。従来の理論的な論理量子ビット数の見積もり(1,000〜1,200量子ビット規模)とは異なり、ハードウェアベンダー自身による「回路コンパイルまで踏み込んだ」見積もりである点で、これまでより具体性の高い参考データです。ただし未査読であることに変わりはなく、実機でこの規模の解読が実証された事実はありません
...
### 格子問題の安全性根拠に関する監視対象の研究

ML-KEM(FIPS 203)・ML-DSA(FIPS 204)はいずれも格子問題(Module-LWE/Module-SIS)の計算困難性に安全性の根拠を置いています。この根拠そのものを対象とする理論研究のうち、監視を継続しているものを記録します。

- **二面体coset問題(DCP)の多項式時間量子アルゴリズム**: AWS暗号研究グループのDaniel R. Simon氏が2026年8月、DCPを多項式時間で解く量子アルゴリズムを提唱しました([IACR ePrint 2026/1591](https://eprint.iacr.org/2026/1591))。DCPは格子問題と関連が指摘される問題です。2026年9月時点で、この結果からML-KEM/ML-DSAへの具体的な攻撃手法や必要資源の見積もりは示されておらず、pqc_railsの実装を変更する必要はありません。ただし格子問題の安全性根拠そのものに関わる理論的動向であるため、査読・検証の進展を継続的に注視します。
...
- [国家量子戦略(改定版)](https://www.whitehouse.gov/presidential-actions/2026/06/ushering-in-the-next-frontier-of-quantum-innovation/) — 米大統領令14413に基づき2026年12月中旬公表予定です。公表前はプレースホルダー参照です
